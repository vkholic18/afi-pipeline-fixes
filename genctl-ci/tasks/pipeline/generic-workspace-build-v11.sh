...
export DISABLE_ARTIFACTORY_PUSH=${DISABLE_ARTIFACTORY_PUSH:-"false"} # By default push images to Artifactory
...
if [[ ${DISABLE_ARTIFACTORY_PUSH} == false ]]; then
...
if [[ $BUILD_ARTIFACT_TYPE == "packages" ]]; then
    export UPLOAD="false"
    export UPLOAD_PACKAGES_MODE="new"
fi
...
if [[ ${UPLOAD} == true ]]; then
    ...
    then
        ...
        # First login to prod and sandbox (skip if DISABLE_ARTIFACTORY_PUSH is true)
        if [[ ${DISABLE_ARTIFACTORY_PUSH} == false ]]; then
            echo ${cc_artif_access_token} | docker login ${artifactory_docker_url} -u ${artifactory_user} --password-stdin
            echo ${cc_artif_access_token} | docker login ${artifactory_sandbox_docker_url} -u ${artifactory_user} --password-stdin
        fi
        ...
        do 
            ...
            # Push the image to artifactory (skip if DISABLE_ARTIFACTORY_PUSH is true and URL is Artifactory)
            if [[ ${DISABLE_ARTIFACTORY_PUSH} == true ]] && [[ ${url_to_push} == *"${artifactory_docker_url}"* || ${url_to_push} == *"${artifactory_sandbox_docker_url}"* ]]; then
                echo "Skipping Artifactory push for ${url_to_push} (DISABLE_ARTIFACTORY_PUSH=true)"
            else
        ...
        if [[ ! -z ${ARTIFACTORY_DOCKER_URL} && ${DISABLE_ARTIFACTORY_PUSH} == false ]]; then
            ...
            if repo_is_from_prod_org ${PATH_TO_WORKSPACE_REPO} || [[ "$(get_system_arch)" == "s390x" ]];
            then
                ...
                ${PATH_TO_GENCTL_CI}/scripts/process_build_meta_v11.sh process_images
            else
            ...
            echo "Skipping Artifactory production push (DISABLE_ARTIFACTORY_PUSH=true)"
        fi
        ...
        if [[ ! -z ${ARTIFACTORY_SANDBOX_DOCKER_URL} && ${DISABLE_ARTIFACTORY_PUSH} == false ]]; then
            ...
            echo "Skipping Artifactory sandbox push (DISABLE_ARTIFACTORY_PUSH=true)"
        fi
...
if [[ "${UPLOAD_PACKAGES_MODE}" = "new" ]]; then
    ...
    if [[ "${SKIP_UPLOAD_PACKAGES}" = "false" ]]; then
        ...
        ${PATH_TO_GENCTL_CI}/scripts/process_build_meta_new/process_build_meta_new_v11.sh "packages" "upload"
    else
    ...
    if [[ ${UPLOAD_DEBIAN} == true ]]; then
        ...
        for deb_push_url in ${DEBIAN_PUSH_URLS}; do
            ...
            if [[ ! -z ${deb_push_url} ]]; then
                ...
                ${PATH_TO_GENCTL_CI}/scripts/process_build_meta_v11.sh process_packages
            fi
        done
    fi
