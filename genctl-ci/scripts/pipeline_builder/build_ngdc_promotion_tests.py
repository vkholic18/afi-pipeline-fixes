...
#
# Description:
#    Parses functional_tests config and outputs test runs to iterate in smotainer
#    Inherited code from pipeline_builder/build_functional_tests.py
#
# Env:
...
#
# Use:
#    python3 build_ngdc_promotion_tests.py
#
...
import os
import argparse
from pipeline_ngdc_cd_config import PipelineConfig
PipelineConfig.PIPELINE_CONFIG_PATH = 'hack/cd/promotion.yaml'

# Constants
CPAP_TEST_FILE_EXTS = ['.py', '.go']
GENERATED_CPAP_CONF_FILE = 'cd_generated_tests.ini'
...
def format_test_execution(config_path, meta_purposes, meta_environment, cmdline, cmdlineraw, iks_cluster_name_override, meta_features, jira_project, api_key_alias, test_plan, processes, skip_test=False):
    """
...
def build_test_executions(configs, test_run_tracker, repo_path=""):
    ...
    for config in configs:
        ...
        executions.append(format_test_execution(config_path, config['meta_purposes'], config['meta_environment'], config['cmdline'], config['cmdlineraw'], config['iks_cluster_name_override'], config['meta_features'], config['jira_project'], config['api_key_alias'], config['test_plan'], config['processes'], skip_test))
...
def parse_env(cli_args):
    ...
    optional_vars = ['WS_PATH', 'TEST_TRACKER_PATH', 'FUNCTIONAL_TESTS_PATH']
...
def main():
    ...
    if args['ws_path']:
        if args['functional_tests_path']:
            pipe_config = PipelineConfig(args['ws_path'], args['functional_tests_path'])
        else:
            pipe_config = PipelineConfig(args['ws_path'], PipelineConfig.PIPELINE_CONFIG_PATH)

        if pipe_config.functional_tests:
            print("Set test executions")
