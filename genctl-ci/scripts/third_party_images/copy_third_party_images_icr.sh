...
# From artifactory sandbox to ICR sandbox
# From artifactory sandbox to ICR prod
...
then
    ...
    export PATH_TO_IMAGES_TO_COPY="${CI_TEMP_DIR}/tmp_third_party_dir_icr"
    ...
    then
        ...
        # Push to ICR sandbox
        export PUSH_REGISTRY=${VPC_ICR_SANDBOX_URL}
        ...
        then
            # This pulls from ICR sandbox and pushes to ICR sandbox with the SHA of PR to dev-int and prefix
            # For example

            # Pull from:
            # us.icr.io/vpc-sandbox-docker-local/sysdig/agent-slim:13.4.0
            # And push to: 
            # us.icr.io/vpc-sandbox-docker-local/third-party-images/monitoring-workspace/sysdig/agent-slim:ecdb58e849472e08321779760baae89397ce623b
            ${PATH_TO_GENCTL_CI}/scripts/copy_images.sh

            # Additional run with prod (Pull from sandbox and push to ICR prod)
            export PUSH_REGISTRY=${IBMCLOUD_CR_URL_ONEPIPELINE}
            ${PATH_TO_GENCTL_CI}/scripts/copy_images.sh

        elif [[ "${PIPELINE_TYPE}" = "dev-integration-merge" ]]
        then
            # This pulls from ICR sandbox and pushes to ICR sandbox with the SHA of merge to dev-int and prefix
            # For example
            ...
            # This pulls from sandbox and pushes to ICR sandbox with the semver of merge to dev-int and prefix
            # For example

            # Pull from:
            # docker-na-public.artifactory.swg-devops.com/wcp-genctl-sandbox-docker-local/sysdig/agent-slim:13.4.0
            # And push to: 
            # us.icr.io/vpc-sandbox-docker-local/third-party-images/monitoring-workspace/sysdig/agent-slim:1.117.0-dev.2
            export PUSH_REGISTRY=${VPC_ICR_SANDBOX_URL}
