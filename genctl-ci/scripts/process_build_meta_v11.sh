...
function process_images(){  
	...
	if [[ -n ${images} ]]; then
		...
		do
			...
			# Handle no-arch images (single-platform without architecture suffix)
			if [[ ${meta_arch} == "no_arch" ]]; then
				echo "Processing no-arch images (single-platform without architecture suffix)"
				meta_image_list=$(yq -r ".images.${meta_arch} | select(. != null) | if type==\"string\" then . else .[] end" ${path_to_build_meta_file})
				
				if [[ ${meta_image_list} == "" ]]; then
					echo "Info: No images defined under no_arch for pushing images"
				fi
				
				# For no-arch, we don't append architecture suffix
				for image in ${meta_image_list}
				do
					echo "Processing no-arch image: ${image}..."
					image_path="${image%/*}"
					short_image_name=$(echo ${image} | sed -e 's/^.*\///')
					tags="${git_hash} ${git_tag}"
					# Push without architecture suffix by using "no-arch" as meta_arch
					push_to_docker_repository ${DOCKER_REGISTRY_PUSH_URL} ${image_path} ${short_image_name} "no-arch" "${tags}"
				done
			...
			elif [[ ${meta_arch} == $(get_system_arch) ]] || [[ ${meta_arch} == "multi_arch" && $(get_system_arch) == "s390x" ]]; then
			# Get list of images under the arch, if "null" -> empty string
				...
				if [[ ${meta_image_list} == "" ]]; then # If the architecture doesn't have any values specified in it
					echo "Info: No images defined under ${meta_arch} for pushing images"
				fi
				...
				do
					...
					short_image_name=$(echo ${image} | sed -e 's/^.*\///')
					tags="${git_hash} ${git_tag}"
					push_to_docker_repository ${DOCKER_REGISTRY_PUSH_URL} ${image_path} ${short_image_name} ${meta_arch} "${tags}"
				done
			# If the system we're using doesn't match one of the conditions
			# This is for multi_arch images building on amd64 machine
			else
				if [[ ${meta_arch} == "multi_arch" && $(get_system_arch) == "amd64" ]]; then
					meta_image_list=$(yq -r ".images.${meta_arch} | select(. != null) | if type==\"string\" then . else .[] end" ${path_to_build_meta_file})
					meta_arch="amd64"
					if [[ ${meta_image_list} == "" ]]; then # If the architecture doesn't have any values specified in it
						echo "Info: No images defined under ${meta_arch} for pushing images"
					...
					echo "Skipping ${meta_arch} - not applicable for current system architecture"
				fi
			fi
  	done
	else
...
# This function processing of the images manifest
function process_image_manifests(){
	source ${PATH_TO_GENCTL_CI}/onepipeline/scripts/common/artifact_utils.sh
	...
	if [[ -n ${images} ]]; then
		...
		for meta_arch in $(yq -r '.images | keys | .[]' ${path_to_build_meta_file})
		   do
			# Always say the build-yaml specified architecture we're processing
			echo "Creating image manifest for ${meta_arch}"
			
			# Skip manifest creation for no_arch images (single-platform without architecture suffix)
			if [[ ${meta_arch} == "no_arch" ]]; then
				echo "Info: No images defined under multi_arch for creating manifest"
				continue
			fi
			...
			tags="${git_hash} ${git_tag}"
		     if [[ ${meta_image_list} == "" ]]; then # If the architecture doesn't have any values specified in it
				echo "Info: No images defined under ${meta_arch} for creating manifest"
			else
			  create_docker_manifest ${DOCKER_REGISTRY_PUSH_URL} "${meta_image_list}" "$tags" ${meta_arch}
		     fi
		done
	else
		echo "No images specified to create manifest"
	fi
}
