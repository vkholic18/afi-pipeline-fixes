...
# Some variables
supported_process_modes="upload download_and_save_artifacts move_from_pre_release_to_vetted move_from_vetted_to_final_destination copy_from_pre_release_to_final_destination"
...
then
    ...
    then
        ...
        GIT_SHA="${SHA_TO_USE_FOR_SEARCH_PACKAGES}"
            ...
            for package_type in ${package_types}; do
                ...
                then
                    ...
                    do
                        ...
                        if [[ ${meta_arch} == "arm64" && $(get_system_arch) == "amd64" ]]; then
                            meta_package_list=$(yq -r ".packages.${package_type}.${meta_arch} | select(. != null) | if type==\"string\" then . else .[] end" "${PATH_TO_BUILD_META}")
                        elif [[ ${meta_arch} == "noarch" && $(get_system_arch) == "amd64" ]]; then
                            meta_package_list=$(yq -r ".packages.${package_type}.${meta_arch} | select(. != null) | if type==\"string\" then . else .[] end" "${PATH_TO_BUILD_META}")
                        elif [[ ${meta_arch} == $(get_system_arch) ]]; then
                            meta_package_list=$(yq -r ".packages.${package_type}.${meta_arch} | select(. != null) | if type==\"string\" then . else .[] end" "${PATH_TO_BUILD_META}")                                                
                        elif [[ "${process_mode}" != "upload" ]] && [[ ${meta_arch} == "s390x" && $(get_system_arch) == "amd64" ]]; then
                            meta_package_list=$(yq -r ".packages.${package_type}.${meta_arch} | select(. != null) | if type==\"string\" then . else .[] end" "${PATH_TO_BUILD_META}")                    
                        else
                            meta_package_list=""
                        fi
                        ...
                        if [[ ${meta_package_list} != "" ]]; then # If the architecture doesn't have any values specified in it
                            ...
                            then
                                ...
                                do
                                        ...
                                        then
                                            ...
                                            if [[ -n ${result} ]]; then
                                                ...
                                                while IFS= read -r found_file; do
                                                    found_file_name=$(echo "$found_file" | sed 's|^./||')                                                                                                                                                            
                                                    echo "Found local file ${found_file_name}, in directory ${PWD}"
                                                    
                                                    # Construct full Artifactory path
                                                    ...
                                                    if [[ $PROCESS_BUILD_META_UPLOAD_PACKAGES_INCLUDE_METADATA = true ]]; then
                                                        echo "Including metadata for package ${found_file_name}"
                                                    ...
                                                    if [[ $PROCESS_BUILD_META_DRY_RUN_MODE = true ]]; then
                                                        ...
                                                        echo "Uploading ${found_file_name} to ${full_path_in_artifactory}"
                                    ...
                                    elif [[ "${process_mode}" == "copy_from_pre_release_to_final_destination" ]]
                                    then
                                        echo "### MOVE FROM PRE-RELEASE TO FINAL DESTINATION ###"
                                        
                                         # First get the name
                                        get_file_name_in_artifactory "${CC_ARTIF_ACCESS_TOKEN}" "${ARTIFACTORY_BASE_URL}" \
                                        "${pre_release_path_after_url}" "${file_name_to_search}"

                                        # Then move from pre-release to final destination
                                        from="${pre_release_path_after_url}/${FOUND_FILE_NAME_IN_ARTIFACTORY}"
                                        ...
                                        if [[ $PROCESS_BUILD_META_DRY_RUN_MODE = true ]]; then
                                            ...
                                            copy_in_artifactory ${CC_ARTIF_ACCESS_TOKEN} ${ARTIFACTORY_BASE_URL} ${from} ${to}
                                        fi
