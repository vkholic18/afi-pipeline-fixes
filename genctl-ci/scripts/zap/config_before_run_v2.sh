...
python3 ${PATH_TO_GENCTL_CI}/scripts/zap/swagger_def_v2.py --category_name ${CATEGORY_NAME} --api_file_name ${API_FILE_NAME} --endpoints ${ENDPOINTS} --profiles ${PROFILES} 
...
set_env "zap-api-custom-script" "one-pipeline-config-repo/scripts/zap/custom_api_script_v2.sh" 

echo "config run before successful"
