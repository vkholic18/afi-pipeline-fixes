...
# If required, the files are uploaded
# This script is intended to be used in the merge to dev-integration pipeline (In Razee) / merge pipeline for non-razee

# Important: 

# This script uploads to a pre_release directory, this is because at this stage we are not sure yet that all the pipeline passed
# At the end of the merge to dev-integration pipeline, files are moved from this pre_release directory to the relevant location
...
if [[ "${PIPELINE_TEMPLATE_TYPE}" == "razee" ]] || [[ "${PIPELINE_TEMPLATE_TYPE}" == "globals" ]] || [[ "${PIPELINE_TEMPLATE_TYPE}" == "hotfix-razee" ]]
then
    # This is used to set the artifact & provenance field for deployment, the reason is that we need to set the path that eventually the file will have
    ...
    if [ -d "${PATH_TO_WORKSPACE_REPO}/${COS_UPLOAD_CONTENT_ROOT}" ]; then
        ...
        then
            ...
            echo "It was explicitly required not to create ZIP and deployment metadata"
        fi
    else 
        echo "Warning: Could not find deployment files, ZIP and metadata JSON file won't be created"
    fi
fi
...
check_before_upload "${PIPELINE_TEMPLATE_TYPE}"
...
then
    ...
    if [[ $CREATE_AND_UPLOAD_CANDIDATE_FILES_DRY_RUN = true ]]; then
        echo "DRY RUN MODE !!! - We would have uploaded to: ${URL_TO_UPLOAD_JSON}..."
        cat ${JSON_FILE_NAME}
    else
