...
DISABLE_ARTIFACTORY_PUSH=${DISABLE_ARTIFACTORY_PUSH:-"false"} # By default save artifacts from Artifactory
    ...
    then
        ...
        then
            ...
            if [[ ! ${images} == null ]]; then
                ...
                # Login to artifactory and save artifacts (skip if DISABLE_ARTIFACTORY_PUSH is true)
                if [[ ${DISABLE_ARTIFACTORY_PUSH} == false ]]; then
                    ...
                    echo "Skipping Artifactory artifact save (DISABLE_ARTIFACTORY_PUSH=true)"
                fi
                ...
                then
                    ...
                    # Use suffix only if we're also pushing to Artifactory
                    if [[ ${DISABLE_ARTIFACTORY_PUSH} == true ]]; then
                        ICR_SUFFIX=" "
                    else
                        ICR_SUFFIX="${SUFFIX_FOR_ICR_SAVE_ARTIFACTS}"
                    fi

                    save_artifacts_images "${PATH_TO_BUILD_META}" \
                    "${IBMCLOUD_CR_URL_ONEPIPELINE}" "${GIT_SHA_FOR_SAVE_ARTIFACTS}" \
                    "${SAVE_ARTIFACTS_ONLY_FIRST_IMAGE_MODE}" "${ICR_SUFFIX}" "${SAVE_ARTIFACTS_FAIL_IF_NO_MANIFEST}"
                fi
                ...
                then
                    ...
                    # Call function that saves artifacts for third party images (skip if DISABLE_ARTIFACTORY_PUSH is true)
                    if [[ ${DISABLE_ARTIFACTORY_PUSH} == false ]]; then
                        ...
                        echo "Skipping Artifactory third party artifact save (DISABLE_ARTIFACTORY_PUSH=true)"
                    fi
        ...
        then
            ...
            if [[ ! ${packages} == null ]]; then
                if [[ "${PIPELINE_TEMPLATE_TYPE}" == "razee" && "${PIPELINE_TYPE}" == "pr" ]]; then
                    export SHA_TO_USE_FOR_SEARCH_PACKAGES="${RESULT_DEV_INT_SHA}"
                fi
                ${PATH_TO_GENCTL_CI}/scripts/process_build_meta_new/process_build_meta_packages_new_v11.sh "download_and_save_artifacts"
            else
