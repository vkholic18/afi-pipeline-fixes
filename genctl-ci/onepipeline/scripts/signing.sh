...
SIGNING_TRIGGER_TO_USE=${SIGNING_TRIGGER_TO_USE:-"taas-worker-trigger"}
...
if [[ $SKIP_ONE_PIPELINE_SIGNING = true ]]; then
    ...
    ${PATH_TO_GENCTL_CI}/onepipeline/scripts/trigger_subpipeline.sh ${SIGNING_STAGE_NAME} ${SIGNING_TRIGGER_TO_USE}
fi