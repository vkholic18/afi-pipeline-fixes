...
export CATEGORY_NAME=$4
export API_FILE_NAME=$5
export PROFILES=$6
export ENDPOINTS=$7
export CUSTOM_SUB_PIPELINE_CONFIG=$8
# $9 is optional; defaults to empty JSON array when not provided
export EXCLUDE_ENTRIES=${9:-"[]"}

set_env category-name "${CATEGORY_NAME}"
set_env api-file-name "${API_FILE_NAME}"
set_env profiles "${PROFILES}"
set_env endpoints "${ENDPOINTS}"
set_env one-pipeline-config "${CUSTOM_SUB_PIPELINE_CONFIG}"
set_env exclude-entries "${EXCLUDE_ENTRIES}"

export_env "category-name"
export_env "api-file-name"
export_env "profiles"
export_env "endpoints"
export_env "one-pipeline-config"
export_env "exclude-entries"
...
# set_env_for_subpipeline_for_dynamic_scan
...
if [ "${WAIT_UNTIL_FINISHES}" == "true" ]; then
    ...
    for TASK in $(echo ${PENDING_TASKS} | tr "\n" " "); do
      ...
      source "${ONE_PIPELINE_PATH}"/iam/get_token
