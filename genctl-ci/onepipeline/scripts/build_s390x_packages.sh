...
# PATH_TO_GENCTL_CI, IBMCLOUD_KEY, SKIP_BUILD_S390X_PACKAGES

# In addition the following variables are optional and if not have values they will take the default

BUILD_S390X_STAGE_NAME=${BUILD_S390X_STAGE_NAME:-"build-s390x-packages-as-subpipeline"}
BUILD_S390X_TRIGGER_TO_USE=${BUILD_S390X_TRIGGER_TO_USE:-"build-s390x-packages"}

#This can be removed once we get new agent
CUSTOM_SUB_PIPELINE_CONFIG=$1

if [[ $SKIP_BUILD_S390X_PACKAGES = true ]]; then
    echo "Skipping..."
else
