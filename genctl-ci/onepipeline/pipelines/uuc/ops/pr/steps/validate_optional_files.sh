...
# Optional Files Validation Script Wrapper
# This script validates optional files from repositories based on onboarding.yaml
...
# Usage: ./validate_optional_files.sh [--debug] [<path_to_onboarding.yaml> ...]
...
main() {
    ...
    for yaml_file in "${files_to_validate[@]}"; do
        ...
        run_validation_on_file "Validating Optional Files" "$yaml_file" "$SCRIPT_DIR/validate_optional_files.py" "$DEBUG_FLAG"
    ...
    if [ $overall_exit_code -eq 0 ]; then
        echo -e "${GREEN}✓ All optional files validation checks passed${NC}"
    else
        echo -e "${RED}✗ Some optional files validation checks failed${NC}"
