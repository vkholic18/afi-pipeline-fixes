...
# Mandatory Files Validation Script Wrapper
# This script validates mandatory files existence, executable permissions, and empty file checks
# based on onboarding.yaml configuration
...
# Usage: ./validate_mandatory_files.sh [--debug] [<path_to_onboarding.yaml> ...]
...
main() {
    ...
    for yaml_file in "${files_to_validate[@]}"; do
        ...
        run_validation_on_file "Validating Mandatory Files" "$yaml_file" "$SCRIPT_DIR/validate_mandatory_files.py" "$DEBUG_FLAG"
    ...
    if [ $overall_exit_code -eq 0 ]; then
        echo -e "${GREEN}✓ All mandatory files validation checks passed${NC}"
    else
        echo -e "${RED}✗ Some mandatory files validation checks failed${NC}"
