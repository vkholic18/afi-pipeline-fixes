...
# Check if infrastructure provisioning script exists
if [ -f "${SCRIPT_DIR}/provision_team_infrastructure.sh" ]
then
    echo "Running team infrastructure provisioning"
    run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "PROVISION_TEAM_INFRASTRUCTURE" ${EXIT_ON_TASK_FAILURE} \
    ${SCRIPT_DIR}/provision_team_infrastructure.sh
else
    echo "provision_team_infrastructure.sh file not found"
    exit 1
fi
