...
export PATH_TO_RELEASE_ENVIRONMENT=${WORKSPACE}/release-environment # Used in prepare hostos release bundle
...
### Check if we are running on a release bundle of high level typel if yes, run autosemver ###
if [[ "$HIGH_LEVEL_RELEASE_BUNDLE_TYPES" =~ (^|[[:space:]])$COMPONENT($|[[:space:]]) ]]
then
    run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "AUTO_SEMVER" ${EXIT_ON_TASK_FAILURE} \
    ${PATH_TO_GENCTL_CI}/tasks/auto-semver.sh
else
    echo "No need to run AutoSemver"
fi
