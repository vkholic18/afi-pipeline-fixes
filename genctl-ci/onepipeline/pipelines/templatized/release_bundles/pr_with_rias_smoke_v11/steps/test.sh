...
if [[ ${SKIP_NGDC_DEPLOY} == "false" ]]; then  ## add overrides for high level release bundles

    ### ng and ngdc deploy ###
    run_job "NG_AND_NGDC_DEPLOY" ${EXIT_ON_JOB_FAILURE} \
    ${PATH_TO_GENCTL_CI}/onepipeline/jobs/ng_and_ngdc_deploy_v11.sh
else
    if [[ "$PR_USERLOGIN" == "OnePipeLineCI" ]] && [[ "$PR_LABELS" =~ (^|[[:space:]])"${FOR_CI_ONLY_IGNORE_BUILD_UT_LABEL}"($|[[:space:]]) ]]; 
    then
        echo "Skipping deploy_dal as this is just to bump a component"
        exit 0
    else
        ### Deploy Dal with rias smoke tests (If needed) ###
