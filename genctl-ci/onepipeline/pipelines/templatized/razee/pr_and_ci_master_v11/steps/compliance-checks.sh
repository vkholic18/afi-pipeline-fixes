...
cp ${PATH_TO_GENCTL_CI}/hack/ci/custom_required_checks_razee.json ${PATH_TO_WORKSPACE_REPO}/

if [ $? -eq 0 ]; then
    echo "custom_required_checks.json copied successfully"
    set_env cra-bom-generate "0"
    set_env cra-vulnerability-scan "0"
    set_env branch-protection-rules-path "custom_required_checks_razee.json"
else
  echo "custom_required_checks_razee.json file copy failed. Aborting code-compliance-checks"
  exit 1
fi
