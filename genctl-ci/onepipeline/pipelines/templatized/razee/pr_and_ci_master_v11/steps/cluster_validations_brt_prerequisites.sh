...
generate_desired_genctl_ffsld() {
    ...
    echo "cat the ffsld yaml file"
    cat ${PATH_TO_WORKSPACE_REPO}/FFSLD_ARTIFACTS/ffsld/genctl/${MZONE_NAME}/featureflagsetld.yaml

    yq '.data' ${GENCTL_FFSLD_FILE_PATH} | jq '{data: .}' > ${PATH_TO_GENCTL_CI}/genctl_desired_ffsld.json

    echo "cat the genctl_desired_ffsld.json"
    cat "${PATH_TO_GENCTL_CI}/genctl_desired_ffsld.json"
    ...
    export CLUSTER_TO_CHECK="${MZONE_NAME}"
    export FAIL_IF_ERROR=true
...
setup_brt_validation_prerequisites() {
    ...
    # Disable command tracing for helper functions
    set +x
    ...
    echo "=========================================="
    echo "BRT validation prerequisites complete"
    echo "=========================================="
}

disconnect_cos_remote_resource_and_apply_patch() {
    echo "Executing validation commands directly (containerless)..."

    # Set kubeconfig to use the file we retrieved from Vault
    export KUBECONFIG="${KUBECONFIG_FILE}"

    echo "Add the debug label and patch the resource"
    kubectl label ffsld -n genctl genctl-ffs-ld deploy.razee.io/debug="true" --overwrite
    kubectl patch featureflagsetld genctl-ffs-ld -n genctl --type=merge --patch-file ${PATH_TO_GENCTL_CI}/genctl_desired_ffsld.json
    sleep 300
    echo "FFSLD resource patched"
}
...
validate_razee_cluster_genctl() {
    ...
    if [ ${FAIL_IF_ERROR} == "true" ]; then
        set -e
    fi

    echo "executing genctl_post_deploy_readiness"
...
validate_feature_flags() {
    ...
    # Source feature flag validation utilities
    source ${PATH_TO_GENCTL_CI}/scripts/validate_featureflag_utils.sh

    # Install Python requirements
    python3 -m pip install -q -r ${PATH_TO_GENCTL_CI}/scripts/featureflags/requirements.txt

    # Run feature flag validation
    python3 ${PATH_TO_GENCTL_CI}/scripts/featureflags/featureflags_cos_ffsld.py \
        ${LAUNCH_DARKLY_FEATURE_FLAG} \
        validate_promotion_by_ld_with_file \
        ${WORKSPACE}/desired_deployment_versions.json \
        genctl
