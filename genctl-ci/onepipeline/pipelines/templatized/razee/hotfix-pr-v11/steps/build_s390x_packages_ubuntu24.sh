...
if [[ $BUILD_ARTIFACT_TYPE == "packages" && $BASE_IMAGE_VERSION == "ubuntu22" ]]; then
    if check_yaml_path_has_data "packages.debian.s390x"; then
        ### Build ###
        run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "BUILD_S390X_PACKAGES_UBUNTU22" ${EXIT_ON_TASK_FAILURE} \
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_packages.sh "onepipeline/pipelines/templatized/razee/hotfix-pr-v11/.pipeline-config-subpipeline-s390x-artifacts-ubuntu22.yaml"
    else
...
if [[ $BUILD_ARTIFACT_TYPE == "packages" && $BASE_IMAGE_VERSION == "ubuntu24" ]]; then
    if check_yaml_path_has_data "packages.debian.s390x"; then
        ...
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_packages_ubuntu24.sh "onepipeline/pipelines/templatized/razee/hotfix-pr-v11/.pipeline-config-subpipeline-s390x-artifacts-ubuntu24.yaml"
    else
