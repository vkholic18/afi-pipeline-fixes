...
if [[ $BUILD_ARTIFACT_TYPE == "images" && $BASE_IMAGE_VERSION == "ubuntu22" ]]; then
    if check_yaml_path_has_data "images.multi_arch"; then
        ...
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_images.sh "onepipeline/pipelines/templatized/razee/hotfix-pr-v11/.pipeline-config-subpipeline-s390x-artifacts-ubuntu22.yaml"
    else
...
if [[ $BUILD_ARTIFACT_TYPE == "images" && $BASE_IMAGE_VERSION == "ubuntu24" ]]; then
    if check_yaml_path_has_data "images.multi_arch"; then
        ...
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_images_ubuntu24.sh "onepipeline/pipelines/templatized/razee/hotfix-pr-v11/.pipeline-config-subpipeline-s390x-artifacts-ubuntu24.yaml"
    else
