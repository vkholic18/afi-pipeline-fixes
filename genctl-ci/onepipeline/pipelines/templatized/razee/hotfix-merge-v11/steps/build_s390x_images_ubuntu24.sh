...
if [[ $BUILD_ARTIFACT_TYPE == "images" && $BASE_IMAGE_VERSION == "ubuntu22" ]]; then
    if check_yaml_path_has_data "images.multi_arch"; then
        ...
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_images.sh "onepipeline/pipelines/templatized/razee/hotfix-merge-v11/.pipeline-config-subpipeline-s390x-artifacts-ubuntu22.yaml"
    else
        echo "There are no multi_arch images to build on ubuntu 22.04, so skipping..."
    fi
fi

if [[ $BUILD_ARTIFACT_TYPE == "images" && $BASE_IMAGE_VERSION == "ubuntu24" ]]; then
    if check_yaml_path_has_data "images.multi_arch"; then
        ### Build ###
        run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "BUILD_S390X_IMAGES_UBUNTU24" ${EXIT_ON_TASK_FAILURE} \
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_images_ubuntu24.sh "onepipeline/pipelines/templatized/razee/hotfix-merge-v11/.pipeline-config-subpipeline-s390x-artifacts-ubuntu24.yaml"
    else
        echo "There are no multi_arch images to build on ubuntu 24.04, so skipping..."
    fi
fi
