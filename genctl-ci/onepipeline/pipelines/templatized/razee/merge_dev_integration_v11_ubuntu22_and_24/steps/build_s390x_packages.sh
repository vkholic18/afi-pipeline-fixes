...
if [[ $BASE_IMAGE_VERSION == "ubuntu22" ]]; then
    if has_s390x_packages_data; then
        ...
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_packages.sh "onepipeline/pipelines/templatized/razee/merge_dev_integration_v11_ubuntu22_and_24/.pipeline-config-subpipeline-s390x-artifacts-ubuntu22.yaml"
    else
        echo "There are no s390x packages to build, so skipping..."
    fi
fi
...
if [[ $BASE_IMAGE_VERSION == "ubuntu24" ]]; then
    if has_s390x_packages_data; then
        ...
        ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_packages_ubuntu24.sh "onepipeline/pipelines/templatized/razee/merge_dev_integration_v11_ubuntu22_and_24/.pipeline-config-subpipeline-s390x-artifacts-ubuntu24.yaml"
    else
        echo "There are no s390x packages to build, so skipping..."
    fi
fi