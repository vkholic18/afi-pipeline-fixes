...
export PIPELINE_TEMPLATE_TYPE="prod_artifacts"
...
if has_s390x_packages_data; then
    ### Build ###
    run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "BUILD_S390X" ${EXIT_ON_TASK_FAILURE} \
    ${PATH_TO_GENCTL_CI}/onepipeline/scripts/build_s390x_packages.sh "onepipeline/pipelines/templatized/prod_artifacts/merge_v11/.pipeline-config-subpipeline-s390x-artifacts.yaml"
else
