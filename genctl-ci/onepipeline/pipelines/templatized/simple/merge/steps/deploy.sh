...
# In some flows we want to skip the deploy itself, so check 
if [[ $SKIP_DEPLOY = true ]]; then
    echo "Skipping Deploy..."
else
    if [ -f "${PATH_TO_WORKSPACE_REPO}/hack/ci/deploy.sh" ]
    then
        ## Execute deploy ##
        echo "Running deploy.sh from ${PATH_TO_WORKSPACE_REPO}/hack/ci directory"
        run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "DEPLOY" ${EXIT_ON_TASK_FAILURE} \
        ${PATH_TO_WORKSPACE_REPO}/hack/ci/deploy.sh
    else
        echo "deploy.sh file not found in ${PATH_TO_WORKSPACE_REPO}/hack/ci directory"
    fi
fi
