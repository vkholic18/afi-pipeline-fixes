...
# In some flows we want to skip the release itself, so check 
if [[ $SKIP_RELEASE = true ]]; then
    echo "Skipping Release..."
else    
    if [ -f "${PATH_TO_WORKSPACE_REPO}/hack/ci/release.sh" ]
    then
        ## Execute deploy ##
        echo "Running release.sh from the hack/ci directory"
        run_task ${SET_GHE_STATUSES} ${CHECKS_PREFIX} "RELEASE" ${EXIT_ON_TASK_FAILURE} \
        ${PATH_TO_WORKSPACE_REPO}/hack/ci/release.sh
    else
        echo "release.sh file not found in the hack/ci directory"
    fi
fi
